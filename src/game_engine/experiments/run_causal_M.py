...
RESULTS_ROOT = os.path.join(SRC_ROOT, "results", "causal_M")
...
def _make_cfg(
    *,
    ...
    cfg = replace(
        ...
        streak=replace(base_cfg.streak, lam=float(streak_lambda)),
        seed=int(seed),
    )
...
def _build_agent(
    *,
    ...
    if spec.kind != "llm":
        raise ValueError(
            "run_causal_M.py expects an all-LLM lineup. "
...
def run_causal_m_engine(
    *,
    ...
    if not lineup:
        raise ValueError("run_causal_M.py requires a non-empty LLM lineup.")
    ...
    for M in m_values:
        for seed in seeds:
            ...
            try:
                ...
                print(
                    f"[{completed}/{total}] "
                    f"N={N} | M={M} | p={noise_p:.2f} | lam={streak_lambda:.2f} | seed={seed} | "
